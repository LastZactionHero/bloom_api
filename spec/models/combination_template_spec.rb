  ...
  describe 'starting_plants' do
    ...
    it 'sets a list of starting_plants' do
      ...
      combination_template.starting_plants = (1..3).to_a.map do |i|
        OpenStruct.new(tooltip: "Tooltip #{i}", plant: FactoryGirl.create(:plant, permalink: "permalink_#{i}"))
      end

      expect(combination_template[:starting_plants]).to eq('[{"tooltip":"Tooltip 1","permalink":"permalink_1"},{"tooltip":"Tooltip 2","permalink":"permalink_2"},{"tooltip":"Tooltip 3","permalink":"permalink_3"}]')
    end
  end
end
