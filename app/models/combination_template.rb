...
class CombinationTemplate < ApplicationRecord
  def starting_plants
    ...
    plants_json.map do |plant_json|
      OpenStruct.new(tooltip: plant_json['tooltip'], plant: Plant.find_by(permalink: plant_json['permalink']))
    end
  end

  def starting_plants=(plants_arr)
    self['starting_plants'] = plants_arr.map do |plant_data|
      { tooltip: plant_data.tooltip, permalink: plant_data.plant.permalink }
    end.to_json
  end
end
